...
state.update({
    'api_root': os.environ.get('TRAME_APP_API_ROOT', 'https://data.kitware.com/api/v1'),
    'auth_register': True,
    ...
    def __init__(self, title, description, link):
        with html.Div():
            with html.Div(title, classes='headline font-weight-bold mono mt-8'):
                with vuetify.VBtn(href=Headline.REPO_BASE + link, icon='icon', classes='ml-2', title='View Source', target='_blank'):
...
with VAppLayout(server) as layout:
    with vuetify.VContainer():
        ...
        with html.Div(classes='title mb-1'):
            ...
            html.A('Girder Web Components', href='https://gwc.girder.org/', target= '_blank'),
        ...
        with vuetify.VContainer(v_else=True):
            ...
            with vuetify.VRow(), vuetify.VCol(cols=12):
                ...
                with vuetify.VToolbar(color='primary'):
                    gwc.GirderSearch(
                        select=(handle_search_select, '[$event]',)
                    )
